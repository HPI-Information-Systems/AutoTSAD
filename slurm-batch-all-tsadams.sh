...
#  echo "Submitting Job >tsadams ${d} p=1<"
#  sbatch -J "tsadams ${d//\// } p=1" slurm-run-tsadams.sh --n-jobs "${n_jobs}" "${HOME}/data/benchmark-data/univariate/${d}.test.csv"
#  sleep 0.5
#done
#
#for d in "${gutentag_datasets[@]}"; do
#  echo "Submitting Job >tsadams ${d} p=1<"
#  sbatch -J "tsadams GutenTAG ${d//\// } p=1" slurm-run-tsadams.sh --n-jobs "${n_jobs}""${HOME}/data/univariate-anomaly-test-cases/${d}/test.csv"
#  sleep 0.5
#done
#
#for d in "${sand_datasets[@]}"; do
#  echo "Submitting Job >tsadams ${d} p=1<"
#  sbatch -J "tsadams SAND ${d//\// } p=1" slurm-run-tsadams.sh --n-jobs "${n_jobs}" "${HOME}/data/sand-data/${d}.csv"
#  sleep 0.5
#done

for d in "${synthetic_datasets[@]}"; do
  echo "Submitting Job >SELECT ${d}<"
  sbatch -J "SELECT ${d//\// } p=1" slurm-run-tsadams.sh --n-jobs "${n_jobs}" "${HOME}/data/autotsad-data/synthetic/${d}.csv"
  sleep 0.5
done
