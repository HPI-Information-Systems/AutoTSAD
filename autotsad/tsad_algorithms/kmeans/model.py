...
class KMeansAD(BaseEstimator, OutlierMixin):
    def __init__(self, k: int, window_size: int, stride: int, random_state: int):
        self.k = k
        self.window_size = window_size
        self.stride = stride
        self.model = KMeans(n_clusters=k, init="k-means++", n_init="auto", random_state=random_state)
    ...
    def predict(self, X: np.ndarray, preprocess=True, postprocess=True) -> np.ndarray:
        if preprocess:
            X = self._preprocess_data(X)
