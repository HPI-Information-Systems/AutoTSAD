...
from autotsad.util import format_time, mask_to_slices
...
method_type_colors = {
    ...
    "AutoTSAD": cm(6),
    "AutoTSAD Top-1": cm(4),
    "AutoTSAD Ensemble": cm(5),
}


def _reset_rc_params() -> None:
...
def _save_figure(
    save_files_path: Optional[Path],
    name: str,
    fig: Figure,
    legend: Optional[Union[Legend, Sequence[Artist]]] = None,
) -> None:
    if save_files_path is not None:
        if legend is not None:
            if not isinstance(legend, Sequence):
                legend = [legend]
            fig.savefig(
                save_files_path / f"{name}.pdf",
                bbox_extra_artists=legend,
                bbox_inches="tight",
            )
            fig.savefig(
                save_files_path / f"{name}.png",
                bbox_extra_artists=legend,
                bbox_inches="tight",
            )
        else:
...
def plot_quality(
    ...
    for m in df_quality["Method"].unique():
        ...
        if show_algo_type:
            if "minimum-influence" in m:
                ...
                label += "$^{\\uparrow}$"
            elif "cae-ensemble" in m:
                label += "$^{\\otimes}$"

        if show_success_rate:
            # plot original label as text left-aligned and use success rate as new label
            a_text = plt.text(
                text_offset, method_order[::-1].index(m), label, ha="left", va="center"
            )
        ...
        plt.boxplot(
            ...
            showmeans=True,
            widths=0.8,
            whis=(0.0, 100.0),  # whiskers at min/max
            ...
            labels=[label],
            positions=[method_order[::-1].index(m)],
        )
    ...
    lines = [
        ...
        Rectangle(
            ...
            alpha=0.5,
            linewidth=1,
            color=method_type_colors["Baseline"],
        ),
    ]
    labels = [
        "Median",
    ...
    if "AutoTSAD Top-1" in df_quality["Method Type"].unique():
        lines.append(
            Rectangle(
                ...
                alpha=0.5,
                linewidth=1,
                color=method_type_colors["AutoTSAD Top-1"],
            )
        )
...
@st.cache_data(max_entries=5, show_spinner=False, persist=False)
def plot_quality_by_collection(
    df_quality: pd.DataFrame,
    method_order: Sequence[str],
    metric: str = "range_pr_auc",
    gr_include_oracle: bool = False,
    gr_include_ensembles: bool = False,
    gr_include_selection: bool = False,
    gr_include_kmeans: bool = False,
    use_median: bool = False,
) -> Figure:
    _reset_rc_params()
    figure_width = FIGURE_WIDTH
    colors = [cm(i) for i, _ in enumerate(method_order)][::-1]

    df_tmp = df_quality.copy()
    if not gr_include_oracle:
        df_tmp = df_tmp[~df_tmp["Method"].str.contains("best-algo")]
    if not gr_include_ensembles:
        df_tmp = df_tmp[
            ~df_tmp["Method"].isin(
                ["cae-ensemble", "select-horizontal", "select-vertical"]
            )
        ]
    if not gr_include_selection:
        df_tmp = df_tmp[~df_tmp["Method"].isin(["top-1", "tsadams-mim"])]
    if not gr_include_kmeans:
        df_tmp = df_tmp[~df_tmp["Method"].str.contains("k-Means")]

    df_tmp["Collection"] = df_tmp["Dataset"].str.split(" ").str[0]
    agg_type = "median" if use_median else "mean"
    df_tmp = df_tmp.groupby(["Collection", "Method"])[["Method Type", metric]].agg(
        {metric: agg_type, "Method Type": "first"}
    )
    method_type_mapping = dict(
        (m, t)
        for m, t in df_tmp.reset_index()[["Method", "Method Type"]]
        .drop_duplicates()
        .values
    )
    df_tmp = df_tmp.reset_index().pivot(
        index="Collection", columns=["Method"], values=metric
    )
    index = np.arange(df_tmp.index.shape[0])
    bar_width = 0.6 / len(method_order)
    fig = plt.figure(figsize=(figure_width, 3), dpi=FIGURE_DPI)
    ax = plt.gca()
    for i, method in enumerate(method_order[::-1]):
        if method not in df_tmp.columns.get_level_values(0).unique():
            continue
        method_type = method_type_mapping[method]
    ...
    ax.set(axisbelow=True)
    ax.xaxis.grid(True, linestyle="-", which="major", color="lightgrey", alpha=0.5)
    ax.xaxis.set_ticks(index, df_tmp.index, rotation=45, ha="right")
    spines = ax.spines
    spines["top"].set_visible(False)
    spines["right"].set_visible(False)
    legend = fig.legend(
        loc="lower center", bbox_to_anchor=(0.5, 1), ncol=2, borderaxespad=0.0
    )
    return fig


@st.cache_data(max_entries=2, show_spinner=False, persist=False)
def plot_runtime_traces(
    df_runtime_trace: pd.DataFrame,
    ...
    fig, axs = plt.subplots(
        1, len(trace_datasets), sharey="row", figsize=(FIGURE_WIDTH, 3), dpi=FIGURE_DPI
    )
    ...
    for i, d in enumerate(trace_datasets):
        ...
        for n_jobs in df_runtime_trace.loc[
            df_runtime_trace["dataset_name"] == d, "n_jobs"
        ].unique():
            df = df_runtime_trace[
                (df_runtime_trace["dataset_name"] == d)
                & (df_runtime_trace["n_jobs"] == n_jobs)
            ]
            ...
            df["trace_name"] = (
                df["trace_name"].str.split("-%-").str[-1].apply(adjust_names)
            )
    ...
    n = len(
        [t for t in df_runtime_trace["trace_name"].unique() if t not in exclude_traces]
    ) + len(group_traces)
...
@st.cache_data(max_entries=2, show_spinner=False, persist=False)
def plot_runtime_traces_new(
    df_runtime_trace_default: pd.DataFrame,
    ...
    def _relative_runtime(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        df_autotsad = df[df["trace_name"] == "autotsad"].set_index(
            ["dataset_name", "n_jobs"]
        )["runtime"]
        ...
        df = df.pivot(
            index=["dataset_name", "n_jobs"], columns="trace_name", values="runtime"
        )
        ...
        df["position"] = df[["dataset_name", "n_jobs", "trace_name"]].apply(
            lambda x: df_orig.loc[tuple(x.values), "position"], axis=1
        )
    ...
    df_runtime_trace_default, df_runtime_default = _relative_runtime(
        df_runtime_trace_default
    )
    ...
    datasets = sorted(
        set(df_runtime_trace_default["dataset_name"].unique())
        & set(df_runtime_trace_hpo["dataset_name"].unique())
    )
    for i, d in enumerate(datasets):
        df_default = df_runtime_trace_default[
            df_runtime_trace_default["dataset_name"] == d
        ]
        ...
        axs[0].text(0.5 + 2 * i, 110, label, ha="center", va="bottom")
        axs[0].text(
            i * 2, 105, runtime_hpo, ha="center", va="center", fontsize=FONT_SIZE - 4
        )
        axs[0].text(
            i * 2 + 1,
            105,
    ...
    df_all_runtime_traces = (
        df_all_runtime_traces.groupby(["trace_name", "n_jobs"])[["position", "runtime"]]
        .agg(runtime_agg)
        .reset_index()
    )
    ...
    n = len(
        [
            t
            for t in df_all_runtime_traces["trace_name"].unique()
            if t not in exclude_traces
        ]
    ) + len(group_traces)
    lines, labels = axs[0].get_legend_handles_labels()
    lines = lines[:n][::-1]
...
@st.cache_data(max_entries=2, show_spinner=False, persist=False)
def plot_optimization_scaling(
    df_optimization_variance: pd.DataFrame,
    truncate_labels_to: Optional[int] = None,
    metric_name: str = "range_pr_auc",
    save_files_path: Optional[Path] = SAVE_PATH,
    name: str = "optimization-variance",
) -> Figure:
    _reset_rc_params()
    ...
    for i, dataset in enumerate(df_optimization_variance["dataset"].unique()):
        ...
        axs[0].plot(
            df["max_trials"], df["mean"], color=cm(i + 1), marker=mm(i + 1), label=label
        )
        axs[0].fill_between(
            df["max_trials"],
            df["mean"] - df["std"],
    ...
    legend = axs[0].legend(
        loc="lower center", ncol=1, bbox_to_anchor=(0.5, 1.1), borderaxespad=0.0
    )
    _save_figure(save_files_path, name, fig, legend)
    return fig
...
@st.cache_data(max_entries=2, show_spinner=False, persist=False)
def plot_optimization_improvement(
    df_improvements: pd.DataFrame,
    metric_name: str = "range_pr_auc",
    save_files_path: Optional[Path] = SAVE_PATH,
    name: str = "optimization-improvement",
) -> Figure:
    _reset_rc_params()
...
def plot_ensembling_strategies(
    df_quality: pd.DataFrame,
    method_order: Sequence[str],
    metric: str = "range_pr_auc",
    save_files_path: Optional[Path] = SAVE_PATH,
    name: str = "ensembling_comparison",
) -> Figure:
    _reset_rc_params()
...
def plot_ranking_comparison(
    ...
    save_files_path: Optional[Path] = SAVE_PATH,
    name: str = "ranking_example",
) -> Figure:
    _reset_rc_params()

    algorithms = sorted(
        list(
            set(ranking1[0]["algorithm"].unique())
            | set(ranking2[0]["algorithm"].unique())
        )
    )
    ...
    def _plot_single_ranking(
        ...
        for i in range(len(results)):
            ...
            if col == 0:
                text = axs[idx, col].text(
                    -0.05,
                    0.5,
                    label,
                    transform=axs[idx, col].transAxes,
                    ha="right",
                    va="center",
                    color=c,
                )
        ...
        scores = scores.pivot(
            index="time", columns="algorithm_scoring_id", values="score"
        ).values
        ...
        axs[offset, col].plot(
            index,
            combined_score,
            label="Aggregated Scoring",
            color=combined_score_color,
        )
